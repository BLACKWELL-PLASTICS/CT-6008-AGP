...
public class NetGameCountdown : NetMessage
{
    public enum ACTION
    {
        READY = 1,
    ...
    public NetGameCountdown()
    {
        Code = ServerOpCode.GAME_COUNTDOWN;
    }
    public NetGameCountdown(DataStreamReader a_reader)
    {
        Code = ServerOpCode.GAME_COUNTDOWN;
    ...
    {
        NetUtility.C_GAME_COUNTDOWN?.Invoke(this);
    }
    public override void RevievedOnServer(NetworkConnection a_connection)
    {
        NetUtility.S_GAME_COUNTDOWN?.Invoke(this, a_connection);
    }
}